...
export function InvoiceDocument({ data }: InvoiceDocumentProps) {
  ...
  return (
    <div className="invoice-document bg-white h-[11in] w-[8.5in] mx-auto p-3 shadow-sm border border-secondary text-[#1A1A1A] flex flex-col overflow-hidden box-border">
      {/* 1. Organization Details - CENTER ALIGNED */}
      <div className="flex flex-col items-center text-center mb-3">
        <div className="relative w-12 h-12 mb-1 grayscale">
          <Image 
        ...
        <h1 className="text-lg font-headline font-bold text-primary mb-0.5 uppercase tracking-wider">{BUSINESS_DETAILS.name}</h1>
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="text-left">
          <h2 className="text-xl font-headline italic text-secondary opacity-50 uppercase tracking-tighter mb-0.5">INVOICE</h2>
        ...
        <div className="text-right max-w-xs">
          <h3 className="text-[8px] font-bold uppercase tracking-widest opacity-60 mb-0.5">Billed To</h3>
          ...
          <tbody className="text-[10px] divide-y divide-secondary/30">
            {data.items.map((item, idx) => (
              <tr key={item.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-[#F5F0E8]/30'}>
                ...
                <td className="py-1 px-2 text-right">GH₵ {item.unitPrice.toFixed(2)}</td>
                <td className="py-1 px-2 text-right font-semibold">GH₵ {(item.quantity * item.unitPrice).toFixed(2)}</td>
              </tr>
            ))}
            {data.items.length === 0 && (
              <tr>
                <td colSpan={4} className="py-2 text-center opacity-40 italic">No items added yet.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="flex flex-col items-end gap-1 mb-3">
        <div className="w-48 space-y-0.5">
          ...
          <div className="flex justify-between items-center pt-0.5 border-t border-secondary mt-0.5">
            ...
            <span className="font-bold text-base text-primary">GH₵ {grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mb-3">
        <h3 className="text-[8px] font-bold uppercase tracking-widest opacity-60 mb-0.5">Notes & Terms</h3>
      ...
      <div className="grid grid-cols-2 gap-4 pt-2 border-t border-secondary/30 mt-auto">
